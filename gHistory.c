...
/**
 * gHistoryFile - gets the history files.
...
/**
 * wrHistoryy - creates a file, or appends to an existing filees.
...
/**
 * rHistory - reads history from files.
 * @info: the parameter struct.
 *
...
int rHistory(info_t *info)
{
	...
	for (i = 0; i < fsize; i++)
		...
		{
			buf[i] = 0;
			bHistoryList(info, buf + last, linecount++);
			last = i + 1;
		}
	if (last != i)
		bHistoryList(info, buf + last, linecount++);
...
/**
 * bHistoryList - adds entry to a history linked list
